...
exports.createPages = async ({ actions, graphql }) => {
  ...
  let siteMapIndex = await axios.get(`https://${withoutTrailingSlash(baseUrl)}/sitemap_index.xml`, {auth: {
  ...
  siteMapIndex = siteMapIndex.replaceAll(`http://${  gatsbyUrl}`, `https://${  gatsbyUrl}`);
  console.log(siteMapIndex);
  await fse.outputFile(path.join(publicPath, `sitemap_index.xml`), siteMapIndex);

  for (const node of downloadableXMLNodes) {
    let mapFromNode = await axios.get(node, {auth: {
        username: 'shesacrowd2',
        password: 'password'
      }});
    ...
    await fse.outputFile(path.join(publicPath, url_parts.pathname), mapFromNode);
  }

  createRedirect({ fromPath: `/sitemap.xml`, toPath: `/sitemap_index.xml`, isPermanent: false, redirectInBrowser: true, force: true });
};
