...
export default function cloneArrayToLength(numOfEls: number, initialArray: unknown[] = []): any[] {
    return [].concat(...Array(numOfEls)).reduce((a: unknown[], _v: never, idx: number) => {
        return [...a, initialArray[idx % initialArray.length]];
    }, []);
}
...
export function cloneArrayToLengthComponents(
    ...
    const els = []
        ...
        .reduce((a: React.ReactElement[], _v: never, idx: number) => {
            return [...a, initialArray[idx % initialArray.length]];
        }, []);
