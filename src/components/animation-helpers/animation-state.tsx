...
class AnimationStateStore extends GlobalState<AnimationState> {
    ...
    initialRun = () => {
        if (typeof window !== 'undefined') {
            ...
            this.setState({ renderedOnce: true, viewportWidth: this.getViewportWidth(), viewportHeight: this.getViewportHeight(), pageHeight: this.getPageHeight() })
        }
    }
    ...
    getPositionOfElement = (ref) => {
        ...
        if (ref.current) {
            ...
            return { top, right, bottom, left, width, height, x, y, pageTop, pageBottom };
        }
...
export const useScrollTrigger = (depth = 1) => {
    ...
    useAnimatableLayoutEffect(() => {
        ...
        if (containerRef?.current && childRef?.current) {
            ...
            let pS = childPos.height < AnimationState.getViewportHeight() ? 'sticky' : 'relative';
            setPositionStyle(pS)
    ...
    return { containerRef, childRef, progress, childWidth, childHeight, containerStyles: { style: { height: childHeight < AnimationState.getViewportHeight() ? `${childHeight * depth}px` : 'auto' }}, childStyles: { style: {position: positionStyle, top: childHeight > viewportHeight ? "30%" : `${(viewportHeight / 2) - (childHeight / 2)}px` }} }
...
export const Parallax = ({x, y, children}) => {
    ...
    const transform = useTransform(scrollY, [Math.max(pageTop - AnimationState.getViewportHeight(), 0) , pageBottom], [y[0], y[1]]);
    ...
    return (
        ...
        <Chakra.Box as={motion.div} style={{y: progress}} transform={isUnderDesktop ? "none!important" : "auto"}>{children}</Chakra.Box>
        </div>
    )
}
