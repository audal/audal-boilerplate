...
export interface GatsbyImageSVGFallbackProps extends UseImageProps, Omit<HTMLChakraProps<"img">, keyof UseImageProps>, ImageOptions {}
...
export function GatsbyImageSVGFallback({src, alt, ...props}: GatsbyImageSVGFallbackProps) {
    ...
    if (typeof src === "string") {
        ...
        return <Chakra.Image as={GatsbyImage} alt={alt ? alt : ""} image={src.localFile.childImageSharp.gatsbyImageData} {...props} />
    } else if (src?.localFile?.svgData) {
        // @ts-ignore
        return <Chakra.Box css={{"svg": { width: "100%", height: "100%" }}} height="100%" dangerouslySetInnerHTML={{__html: src.localFile.svgData}} {...props} />
    } else if (src?.localFile?.publicURL) {
        return <Chakra.Image src={src.localFile.publicURL} alt={alt ? alt : ""}  {...props} />
    } else {
