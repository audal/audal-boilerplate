...
async function findTemplate({ isContentNode, isSingle, type, dbId, isAuthor, slug }) {
	let general = `./src/wp-templates/${isContentNode ? nodeTypes.contentNode : nodeTypes.termNode}/${isSingle ? 'single' : 'archive'}/base.js`;
	let group = `./src/wp-templates/${isContentNode ? nodeTypes.contentNode : nodeTypes.termNode}/${isSingle ? 'single' : 'archive'}/${type}.js`;
	let specific = `./src/wp-templates/${isContentNode ? nodeTypes.contentNode : nodeTypes.termNode}/${isSingle ? 'single' : 'archive'}/${`${type} ${dbId}` ? dbId : '--------'}.js`;

	if (slug === null) {
		const defaultArchive = `./src/wp-templates/${isContentNode ? nodeTypes.contentNode : nodeTypes.termNode}/${isSingle ? 'single' : 'archive'}/default-archive.js`;
...
function wordpressLink(link) {
	return link.endsWith('/') && link.length > 1 ? link.substring(0, link.length - 1) : link;
}

async function templator({ webLink, slug, isContentNode, type, id, dbId }, createPage, nodesForPagination = false, postsPerPage = 10, isAuthor = false) {
	const templatePath = await findTemplate({ isContentNode, isSingle: !nodesForPagination, type, dbId, isAuthor, slug });
...
exports.createPages = async ({ actions, graphql }) => {
  ...
  `);
	...
	const postsPerPage = allSettingsQuery.data.wp.allSettings.readingSettingsPostsPerPage;
  ...
  `);
    ...
    `);
		...
		if (contentType.graphqlSingleName !== 'mediaItem') {
			for (const node of contentTypeQuery.data[`allWp${contentType.graphqlSingleName.toTitleCase()}`].nodes) {
				if (node.isFrontPage) {
  ...
  `);
	...
	for (const taxonomyType of taxonomiesQuery.data.allWpTaxonomy.nodes) {
      ...
      `);
		...
		for (const term of termQuery.data[`allWp${taxonomyType.graphqlSingleName.toTitleCase()}`].nodes) {
			await templator(
				{
					webLink: term.uri,
					slug: term.slug,
  ...
  `);
	...
	for (const node of authorsQuery.data.allWpUser.nodes) {
		await templator(
			{ webLink: node.uri, id: node.id, slug: node.slug, isContentNode: false, isSingle: false, type: 'author', dbId: node.databaseId },
			createPage,
			node.posts.nodes,
			postsPerPage,
			true
  ...
  `);
	...
	yoastRedirects.data.wp.seo.redirects.forEach((item) => {
		createRedirect({ fromPath: `/${item.origin}`, toPath: `/${item.target}`, isPermanent: true, force: true, redirectInBrowser: true });
	});
